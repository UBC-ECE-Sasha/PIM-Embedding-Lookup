...
void populate_mram(uint32_t table_id, uint64_t nr_rows, uint64_t nr_cols, int32_t *table_data){
    ...
    if( table_size <= MAX_CAPACITY){
        ...
        //total_buffers++;
        //if(max_buffer_len<table_size*sizeof(int32_t))
            //max_buffer_len=ALIGN(table_size*sizeof(int32_t),8);
        ...
        for(int j=0; j<buffer_per_table[table_id]; j++){
            ...
            /*if(table_size<((j+1)*(MAX_CAPACITY)-1))
                last_indices[total_buffers+j]=table_size-1;
            else
                last_indices[total_buffers+j]=(j+1)*(MAX_CAPACITY)-1;*/
            //total_buffers++;
            indices_len++;
    ...
    for(int j=0; j<buffer_per_table[table_id]; j++){
        first_index=first_indices[total_buffers];
        last_index=last_indices[total_buffers];
    ...
    if(ready_buffers==DPUS_PER_RANK || table_id==NR_TABLES-1){
        ...
        DPU_FOREACH(set, dpu, dpu_id){
            ...
            DPU_ASSERT(dpu_copy_to(set, "row_size_input", 0, (const uint64_t *)&table_rows[allocated_dpus+dpu_id], sizeof(uint64_t)));
            DPU_ASSERT(dpu_copy_to(set, "col_size_input", 0, (const uint64_t *)&table_cols[allocated_dpus+dpu_id], sizeof(uint64_t)));
            ...
            DPU_ASSERT(dpu_copy_to(dpu, DPU_MRAM_HEAP_POINTER_NAME, 0, (const int32_t *)emb_buffer[dpu_id], ALIGN(len*sizeof(int32_t),8)));
            printf("copied %d buffer to dpu\n",dpu_id);
        }
        int32_t ans[2];
        DPU_FOREACH(set, dpu, dpu_id){
            printf("first: %d, 2nd: %d, -1: %d, last: %dfor %d buffer\n",emb_buffer[dpu_id][0],emb_buffer[dpu_id][1],emb_buffer[dpu_id][last_index-first_index-1],emb_buffer[dpu_id][last_index-first_index],total_buffers);
            dpu_launch(dpu, DPU_SYNCHRONOUS);
            first_index=first_indices[allocated_dpus+dpu_id];
            last_index=last_indices[allocated_dpus+dpu_id];
            uint32_t offset= (last_index-first_index+1)*sizeof(int32_t);
            DPU_ASSERT(dpu_copy_from(dpu, DPU_MRAM_HEAP_POINTER_NAME, offset , (int32_t*)ans, 2*sizeof(int32_t)));
            //printf("%d: %d, %d\n",dpu_id,ans[0], ans[1]);
