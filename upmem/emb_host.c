...
void populate_mram(uint8_t set_num, uint32_t dpu_n, uint32_t nr_rows, uint32_t nr_cols, float *data) {

    uint32_t write_len=nr_cols*nr_rows*sizeof(uint32_t);
    if(write_len%8!=0)
    ...
    DPU_ASSERT(dpu_copy_to(set[set_num], "row_size_input", 0, (const uint32_t *)&nr_rows, sizeof(nr_rows)));
    DPU_ASSERT(dpu_copy_to(set[set_num], "col_size_input", 0, (const uint32_t *)&nr_cols, sizeof(nr_cols)));
...
void lookup(uint8_t set_num, float *ans, uint32_t* input,uint32_t length, uint32_t nr_rows, uint32_t nr_cols){

    uint32_t offset=nr_cols*nr_rows*sizeof(uint32_t);
    if(offset%8!=0)
        offset+=8-(offset%8);

    uint32_t write_len=length*sizeof(uint32_t);
    if(write_len%8!=0)
        write_len+=8-(write_len%8);

    uint32_t read_len=length*nr_cols*sizeof(uint32_t);
    if(read_len%8!=0)
        read_len+=8-(read_len%8);

    DPU_ASSERT(dpu_copy_to(set[set_num], "index_len_input", 0, (const uint32_t *)&length, sizeof(length)));
