...
int main() {
    ...
    __dma_aligned uint32_t read_buf[MAX_SIZE*4], write_buf[MAX_SIZE*4];
    ...
    {
        mram_read(mram_offset, read_buf, (nr_rows*nr_cols+index_len)*sizeof(uint32_t));
    }
    ...
    mram_write((const uint32_t *)write_buf,mram_offset,index_len*nr_cols*sizeof(uint32_t));
